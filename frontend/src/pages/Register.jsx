...
export default function Register() {
  ...
  // Background slides advance on their own — this is a pitch, not a tutorial to click through.
  useEffect(() => {
    if (showForm) return;
  ...
  return (
    <div className={`h-full flex flex-col font-sans relative overflow-hidden transition-colors duration-500 ${t.bg}`}>
      ...
      <div className="px-8 pb-10 shrink-0 relative z-10">
        ...
        <div className={`text-center mt-5 text-[13px] font-medium ${t.body}`}>
          Already have an account?{" "}
          <Link to="/login" className={`font-bold hover:underline ${t.accent}`}>Log in</Link>
        </div>
      </div>
          ...
          <>
            ...
            >
              <div className="flex items-center justify-between mb-5">
                <div className="font-bold text-[17px] text-ink">Create your account</div>
                <button
                  type="button"
                  onClick={() => setShowForm(false)}
